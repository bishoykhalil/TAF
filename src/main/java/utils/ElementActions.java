...
//    @Step("Click on element: [{elementLocator}]")
	...
	try {
	    ...
	    LoggerClass.logStep(toe.getMessage());
	    fail(toe.getMessage());
	} catch (Exception e) {
	...
	try {
		...
		LoggerClass.logStep("[Element Action] Click on element [" + elementLocator + "]");
	    }
		...
		LoggerClass.logStep(exception.getMessage());
		LoggerClass.logStep(rootCauseException.getMessage());
...
//    @Step("Type data: [{data}] on element: [{elementLocator}]")
	...
	try {
		...
		LoggerClass.logStep("[Element Action] Clear and Type [" + text + "] on element [" + elementLocator + "]");
		...
		LoggerClass.logStep("[Element Action] Type [" + text + "] on element [" + elementLocator + "]");
		// We type here! :D
		driver.findElement(elementLocator).sendKeys(text);
		// Type using JavascriptExecutor in case of the data is not typed successfully
		// using the Selenium sendKeys method
	...
	try {
	    ...
	    LoggerClass.logStep("[Element Action] Select [" + option + "] on element [" + elementLocator + "]");
	    assertFalse(s.isMultiple());
	    switch (selectType) {
	    case TEXT -> s.selectByVisibleText(option);
	    case VALUE -> s.selectByValue(option);
	    default -> LoggerClass.logMessage("Unexpected value: " + selectType);
	    }
	} catch (Exception e) {
	    LoggerClass.logStep(e.getMessage());
	    fail(e.getMessage());
